...
searchInput.addEventListener("keyup", (e)=>{
    ...
    const filtroHeroes = heroes.filter(hero => hero.toLowerCase().includes(e.target.value.toLowerCase()))
    lista.innerHTML = heroesList(filtroHeroes)
})

function heroesList (heroes) {   //las function se ejecutan antes que todo
    let content = heroes
        .map(hero => {
            return `<li>${hero}</li>`
        })
